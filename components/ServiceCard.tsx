...
import {
  ...
  cardBeforeStyles,
  buttonStyles,
} from '../components/styles/seoCards';
...
const ServiceCard = function ServiceCard({ translations }: ServiceCardProps) {
  ...
  return (
    <Card
      className={`group ${cardBaseStyles} ${cardHoverStyles} ${cardBeforeStyles}`}
    >
      <CardBody className="relative z-10">
        ...
        <ul className="list-disc pl-4 space-y-2">
          {translations.items.map((text, i) => (
            <li
              key={i}
              className="text-sm text-slate-200 dark:text-slate-300 transition-colors duration-200"
            >
              {text}
            </li>
          ))}
      ...
      <CardFooter className="relative z-10">
        <Button className={buttonStyles} onPress={handleAuditRequest}>
